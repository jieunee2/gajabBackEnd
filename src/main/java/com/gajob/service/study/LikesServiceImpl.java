...
public class LikesServiceImpl implements LikesService {
  ...
  public String likes(Long studyId) {
    ...
    if (
      ...
      likesRepository.save(new Likes(study, user));
      return "increase-likes";
    }
    // 이미 좋아요를 눌렀던 게시물일 경우, 다시 좋아요 기능을 요청할 경우 좋아요 해제
    else if (likesRepository.findByUserAndStudy(user, study).get() != null) {
      likesRepository.deleteByUserAndStudy(user, study);
  ...
  private boolean isNotAlreadyLike(User user, Study study) {
    return likesRepository.findByUserAndStudy(user, study).isEmpty();
  }
